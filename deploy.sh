...
PYTHON_CMD="python3"
# 进程PID存储文件
PID_FILE="${LOG_DIR}/rss_monitor.pid"
# ===================== 函数定义 =====================
...
# 启动进程
start_process() {
    # 按日期生成日志文件名（格式：YYYY-MM-DD-rss-monitor.log）
    LOG_FILE="${LOG_DIR}/$(date +%Y-%m-%d)-rss-monitor.log"
    # 错误日志重定向到同日期的error.log
    ERROR_LOG_FILE="${LOG_DIR}/$(date +%Y-%m-%d)-rss-monitor-error.log"
...
check_status() {
    if [ -f "${PID_FILE}" ]; then
        ...
        if ps -p ${CURRENT_PID} &> /dev/null; then
            ...
            echo "日志目录：${LOG_DIR}"
            echo "最新日志：${LOG_DIR}/$(date +%Y-%m-%d)-rss-monitor.log"
        else
